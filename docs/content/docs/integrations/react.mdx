...
```
The inferred type of `Check` cannot be named without a reference to `...`. This is likely not portable. A type annotation is necessary.
```

To fix this, you can add a type annotation to the `createComponents` function:

```ts
import { createPermix } from 'permix'
import { createComponents, type PermixComponents } from 'permix/react'

// It should be defined only as type, interface won't work
