...
await permix.checkMiddleware('post', ['read', 'update'])({ req, res, next })
...
http.createServer((req, res) => {
  const permix = permix.get(req, res)
  ...
  if (permix) {
    ...
    if (permix.check('post', 'read')) {
      ...
      res.statusCode = 403
      res.setHeader('Content-Type', 'application/json')
      res.end(JSON.stringify({ error: 'You do not have permission to read posts' }))
    }
  }
})
```
...
- For a generic server framework, use [Permix Server integration](/docs/integrations/server)
