...
```vue title="page.vue"
<template>
  <Check entity="entity" action="action" :data="data">
    Will show this if you have permission
    <template #else>
      Will show this if you don't have permission
    </template>
  </Check>
</template>
```
...
<template>
  <div>
    ...
    <Check name="post" action="edit" :data="props.post">
...
You can find the example of the Vue integration [here](https://github.com/letstri/permix/tree/main/examples/vue-vite).
